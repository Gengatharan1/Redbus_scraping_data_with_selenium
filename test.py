...
print(config)
print(dotenv_values(".env_shared"))


# set background usingabsolute file path
...
if menu_option == "Project":
    ...
    # Ensure 'Origin' and 'Destination' columns exist
    if 'Origin' in df_origin.columns:
        origin_set = df_origin['Origin'].unique().tolist()
    else:
        origin_set = []
        st.error("'Origin' column not found in the origin data.")

    if 'Destination' in df_destination.columns:
        destination_set = df_destination['Destination'].unique().tolist()
    else:
        destination_set = []
        st.error("'Destination' column not found in the destination data.")
